...
| Layered Realization | 多欄 wiring 圖；每條 strand 一個色；step prev/next/play/pause；Top-down ↔ Bottom-up 切換（Gray-path 走法）；Canonical ↔ Reduced 切換（peephole 化簡）；Walk-aware modifier（命中時亮 chip）；runtime verification chip；折疊式「學習資訊」面板含 KaTeX 排版的六段定理與公式 |
