...
3. **Layered Realization** — 自動把含 d ≥ 2 邊的排列分解成多層 legal
   2-cycle，並以 wiring 圖呈現完整路線；可切換 **Canonical**（保留算法
   原始輸出，包含可能的 involution 虛工）與 **Reduced**（消去可相消的對）
...
| Layered Realization | 多欄 wiring 圖；每條 strand 一個色；step prev/next/play/pause；Above ↔ Below 切換；Canonical ↔ Reduced 切換（peephole 化簡）；runtime verification chip |
...
- **Peephole involution cancellation**（`simplifyLayers` in
  `src/lib/layered.ts`，UI 為 Canonical/Reduced toggle）：兩個相同的 legal
  swap 若中間所有層都跟它們 disjoint（可 commute 通過），就可一起消掉，
  迭代至 fixpoint。對應到量子電路就是消去「兩個完全相同的 (n−1)-控制 X」
  ——不論 control 是空心、實心或混雜，只要 pattern 完全一致都會消。
  **不自動套用**，保留 canonical 形式作為教學軌跡。

### 規劃但未實作

- **Smart anchor selection**：cycle decomposition 目前永遠取 `cycle[0]`
  做 anchor、扇出 `(a₀ a₁)(a₀ a₂)…(a₀ a_{m-1})`。改成選擇「到其他 cycle
  成員的 Hamming 距離總和最小」的 anchor，可在化簡前先縮短 Gray-path 總長。
- **Cycle-along-Hamming-path detection**：若整個 cycle 的成員剛好構成一條
  Hamming-1 walk，直接以該 walk 的相鄰邊分解
  `(p₀ p₁)(p₁ p₂)…(p_{m-2} p_{m-1})`，恰 m−1 層、無虛工。`(|00⟩ |01⟩ |11⟩ |10⟩)`
  正是這個 case，現在要靠 reduction 才達標。
- **Per-transposition Above/Below mixing**：目前是全域 toggle。允許每個
  transposition 各自選 above/below，可在 cycle decomposition 邊界爭取
  更多 Gray-path 邊界重合 → 更多自然抵消。
- **Parallel disjoint swaps per layer**：disjoint 的 legal 2-cycle 互相
  commute，可以塞進同一個時間步。這會改變「一層 = 一個 swap」的資料模型，
  需要 wiring 圖與電路圖一併重新設計。降深度（depth）效益最大但工程改動最大。
- **Template matching**：辨識常見 gate 圖樣（Toffoli identity、3-CNOT
  swap…）替換成更短的等效序列。

任何新最佳化都必須通過 `scripts/verify-layered.mjs` 的窮舉＋抽樣＋全
transposition 驗證才能合入。
