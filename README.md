...
3. **Layered Realization** — 自動把含 d ≥ 2 邊的排列分解成多層 legal
   2-cycle，並以 wiring 圖呈現完整路線；可切換 **Original / Reduced**
...
- **Top-down**（內部代號 `above`）— 走 AND-bridge（低權重）：先把 i 獨有
  的 1-bit 清掉，再把 j 獨有的 1-bit 設上；分解形如
  `(p₀ p₁)(p₁ p₂)…(p_{d-1} p_d)…(p₁ p₂)(p₀ p₁)`
- **Bottom-up**（內部代號 `below`）— 走 OR-bridge（高權重）：先設後清；
  分解形如 `(p_{d-1} p_d)…(p₀ p₁)(p₁ p₂)…(p_{d-1} p_d)`
...
### Original vs Reduced layer view

Original 輸出按上述規則「每個 transposition 各自展開」，是結構上對應到
...
例（n = 2，4-cycle `(|00⟩ |01⟩ |11⟩ |10⟩)`，Top-down）：

- Original（5 層）：`(00,01) (00,01) (01,11) (00,01) (00,10)`
- Reduced（3 層）：`(01,11) (00,01) (00,10)` — 達 m-cycle 的層數下界 m−1

> **不自動套用**。Original 讓學習者看見「虛工從哪裡來」（哪一個 Gray-path
...
| Layered Realization | 多欄 wiring 圖；每條 strand 一個色；step prev/next/play/pause；Top-down ↔ Bottom-up 切換（Gray-path 走法）；Original ↔ Reduced 切換（peephole 化簡）；Walk-aware modifier（命中時亮 chip）；runtime verification chip；折疊式「學習資訊」面板含 KaTeX 排版的六段定理與公式 |
...
- **Peephole involution cancellation**（代數恆等式）— UI: `Original / Reduced` toggle
